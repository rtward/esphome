...
unsigned long WiegandReader::get_card_id_(volatile unsigned long *codehigh, volatile unsigned long *codelow, char bitlength) {
...
void WiegandReader::setup() {
  ...
  this->pin_d0_->attach_interrupt(WiegandReader::read_d0_, this, FALLING);
  this->pin_d1_->attach_interrupt(WiegandReader::read_d1_, this, FALLING);
}

std::string WiegandReader::get_code_hex_(std::string rawCode) {
  std::size_t dataStart = rawCode.find_first_not_of("0");
  if (dataStart != std::string::npos) {
    return to_lowercase_underscore(rawCode.substr(dataStart));
  } else {
    return to_lowercase_underscore(rawCode);
  }
}
...
void WiegandReader::update() {
  ...
  if (this->do_wiegand_conversion_()) {
    std::string code = get_code_hex_(uint64_to_string(this->_code));
...
void WiegandReaderTrigger::process(std::string tag) {
